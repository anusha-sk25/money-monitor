...
const ThemeSwitcher = () => {
    const [hue, setHue] = useLocalStorage('money-monitor.color','240')
    const defaultDark= window.matchMedia('(prefers-color-scheme:dark)').matches
    const [theme, setTheme] = useLocalStorage('money-monitor.theme',defaultDark
    ?"dark":"light")
    ...
    return (
        ...
        >
            {
                isColorPicking   //true then add a button n a range slider
                    ? (
